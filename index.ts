...
num2 = 8;
console.log(
  `after performing calculations sum = ${num1 + num2}, sub = ${num1 - num2}, product = ${
    num1 * num2
  }, quotient = ${num1 / num2}`
);
...
let message: string = "Showing error while assigning a number to string.";
console.log(message);

// finding remainder using modulus operator
let num3: number = 6,
  num4: number = 3;
let mod: number = num3 % num4;
...
let A: boolean = true,
  ...
  C: boolean = true;
console.log(
  `(AND) a && b && c = ${A && B && C} -- (OR) a || b || c = ${
    A || B || C
  } -- (NOT) !A = ${!A} !B = ${!B} !C = ${!C}`
);

// using compound assignment operator
let num: number = 10; // Use +=, -=, *=, and /= on this variable.
num += 3;
